...
- (void)didReceiveNotificationRequest:(UNNotificationRequest *)request
    ...
    if (expandableDetails && style && [style isEqualToString:@"CAROUSEL_V1"]) {
        ...
        [[NSMutableArray alloc] initWithCapacity:carouselItems.count];
        ...
        if (carouselItems.count >= 3) {
            ...
            for (NSDictionary *carouselItem in carouselItems) {
                ...
                [self
                 loadAttachmentForUrlString:imageURL
                 atIndex:i
                 completionHandler:^(UNNotificationAttachment *attachment,
                                     NSUInteger idx) {
                     imageDownloadAttemptCounter++;
                     if (attachment) {
                         NSLog(@"Downloaded Attachment No. %ld",
                               (unsigned long)idx);
...
- (void)loadAttachmentForUrlString:(NSString *)urlString
                 ...
                 completionHandler:(void (^)(UNNotificationAttachment *,
