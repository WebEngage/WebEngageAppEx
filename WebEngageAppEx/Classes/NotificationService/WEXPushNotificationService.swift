...
class WEXPushNotificationService: UNNotificationServiceExtension {
    ...
    override func didReceive(_ request: UNNotificationRequest,
        ...
        guard let details = request.content.userInfo["expandableDetails"] as? [AnyHashable: Any] else {
            return deliverContent()
        }

        guard let style = details["style"] as? String else {
            return deliverContent()
        }
...
extension WEXPushNotificationService {
    ...
    func loadCarouselView(with details: [AnyHashable: Any]) {
        ...
        guard let items = details["items"] as? [[AnyHashable: Any]] else {
            return deliverContent()
        }
        ...
        for item in items {
            if let imageStr = item["image"] as? String {
                if let imageURL = URL(string: imageStr) {
                    ...
                    loadAttachment(for: imageURL, at: itemsCounter) { (attachment) in
    ...
    func loadRatingView(with details: [AnyHashable: Any]) {
        ...
        if let imageStr = details["image"] as? String {
            if let imageURL = URL(string: imageStr) {
                loadAttachment(for: imageURL, at: 0) { (attachment) in
                    if let mediaAttachment = attachment {
    ...
    func loadAttachment(for imageURL: URL,
                        at index: Int,
                        completion handler: @escaping (UNNotificationAttachment?) -> Void) {

        let pathExtension = imageURL.pathExtension

        URLSession.shared.downloadTask(with: imageURL) { (temporaryLocation, _, error) in
            ...
            if let error = error {
                ...
                if let temporaryLocation = temporaryLocation {
                    let localURL = URL(fileURLWithPath: temporaryLocation.path.appending(pathExtension))

                    do {

                        try FileManager.default.moveItem(at: temporaryLocation, to: localURL)
